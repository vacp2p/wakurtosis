...
def plot_figure_ex_2(msg_propagation_times, cpu_usage, memory_usage, network_usage, disk_usage, injection_times,
                   simulation_config):
    def style_violin(parts, ax):
    ...
    metrics = [msg_propagation_times, cpu_usage, memory_usage, network_usage, disk_usage, injection_times]
    # Loop through the subplots and plot your data

    for i in range(num_subplots):
        to_plot = metrics[i]

        if type(to_plot) is dict:
            to_plot = to_plot.values()
        parts = axs[i].violinplot(to_plot, showmeans=True)
        axs[i].set_title('Popagation Time (per message)')
        axs[i].set_ylabel('Propagation Time (ms)')
        axs[i].spines[['right', 'top']].set_visible(False)
        axs[i].axes.xaxis.set_visible(False)
        style_violin(parts, axs[i])
