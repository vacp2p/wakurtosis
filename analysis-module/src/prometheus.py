...
def fetch_cadvisor_stats_from_prometheus(container_ip, start_ts, end_ts, prometheus_port=52118):
    # prometheus = subprocess.check_output(
    #     "kurtosis enclave inspect wakurtosis | grep '\\<prometheus\\>' | awk '{print $6}'",
    #     shell=True)
    # url = f'http://{prometheus[:-1].decode("utf-8")}'
    url = 'http://127.0.0.1:50480'
    ...
    net_in = prometheus.custom_query_range(
        f"container_network_receive_bytes_total{{{kurtosis_ip_template}"
        f"= '{container_ip}'}}", start_time=start_timestamp,
        end_time=end_timestamp, step="1s")
    net_in = [int(net_in[0]['values'][i][1]) for i in range(len(net_in[0]['values']))]

    net_out = prometheus.custom_query_range(
        f"container_network_transmit_bytes_total{{{kurtosis_ip_template} "
        f"= '{container_ip}'}}", start_time=start_timestamp,
        end_time=end_timestamp, step="1s")
