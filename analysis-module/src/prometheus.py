...
def get_hardware_metrics(metrics, topology, min_tss, max_tss, prom_port):
    ...
    for container_ip in pbar:
        ...
        try:
            fetch_cadvisor_stats_from_prometheus_by_node(metrics, prometheus, container_ip, min_tss, max_tss)
        except Exception as e:
            analysis_logger.G_LOGGER.error('%s: %s' % (e.__doc__, e))
            continue

    fetch_cadvisor_stats_from_prometheus_by_simulation(metrics, prometheus, container_ips, min_tss, max_tss)


def fetch_cadvisor_stats_from_prometheus_by_simulation(metrics, prom, container_ips, start_ts, end_ts):
...
def fetch_cadvisor_stats_from_prometheus_by_node(metrics, prom, container_ip, start_ts, end_ts):
    ...
    for metric in metrics["by_node"].values():
        stat_function = function_dispatcher[metric["statistic"]]
        if type(metric["metric_name"]) is list:
