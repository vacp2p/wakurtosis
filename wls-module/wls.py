...
def configure_logging(wls_config, config_file):
...
def start_traffic_inyection(wls_config, random_emitters):
    ...
    while True:
        ...
        payload, size = make_payload_dist(dist_type=wls_config['dist_type'].lower(),
                                          min_size=wls_config['min_packet_size'],
                                          max_size=wls_config['max_packet_size'])
        response, elapsed, waku_msg, ts = send_waku_msg(emitter_address, topic=emitter_topic,
        ...
        next_time_to_msg = get_next_time_to_msg(wls_config['inter_msg_type'],
...
def main():
    ...
    configure_logging(wls_config, config_file)
