...
def get_net_metrics(f, lst=[]): #lst might be needed in the future
    f.seek(0)
    rbuff = f.readlines()
    ...
    def procfs_reader(self):
        ...
        for pid in self.docker_pids:
            ...
            try:
                ...
                net = get_net_metrics(self.pid2procfds[pid]["net"]) # Whole row
                blk = get_blk_metrics(self.pid2procfds[pid]["blk"], [0, 1]) # Read, Write
            except:
                continue
            #blk = get_blk_metrics(self.pid2procfds[pid]["blk"], [4, 5]) # Read, Write, SUDO
            out = ( f"SAMPLE_{self.procfs_sample_cnt} "
                    f"{docker_id} {time.time()} "
                    f"{mem} {net} {blk} {cpu}\n"
                  )
