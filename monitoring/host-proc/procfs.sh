...
echo "host-proc: the monitor is waiting on $signal_fifo"
cat  $signal_fifo
...
script_dir=`dirname $odir`
python3 $script_dir/procfs.py --sampling-interval $sinterval --prefix $odir --wls-cid $WAIT_CID  > $proclog 2>&1 &
procfs_pid=$!
...
echo "host-proc: stopping the monitor $procfs_pid"
